...
def flow_control_simulation(protocol: ProtocolsEnum, sequence_of_bits: int, number_of_frames: int,
    ...
    max_sequence_number = (2 ** sequence_of_bits) - 1

    receiver_queue = Queue(maxsize=max_sequence_number)
    receiver_channel_queue = Queue(maxsize=max_sequence_number)

    sender_queue = Queue(maxsize=max_sequence_number)
    sender_channel_queue = Queue(maxsize=max_sequence_number)
    ...
    def receiver_listen_and_forward():
        while True:
            if receiver_channel_queue.full():
